...
interface SettingsModalProps {
  ...
  onSave: (settings: { claudeApiKey: string; webhookUrl: string; customPRPrompt: string }) => void;
  showToast: (msg: string, type?: "success" | "error") => void;
}

export default function SettingsModal({ isOpen, onClose, claudeApiKey, webhookUrl, customPRPrompt, onSave, showToast }: SettingsModalProps) {
  const [claudeKeyDraft,   setClaudeKeyDraft]   = useState(claudeApiKey);
  ...
  const saveClaudeKey = async () => {
    ...
    onSave({ claudeApiKey: k, webhookUrl, customPRPrompt });
  ...
  const saveWebhook = async () => {
    ...
    onSave({ claudeApiKey, webhookUrl: u, customPRPrompt });
      ...
      <div className="card modal-panel" style={{ maxWidth:"640px",width:"100%",padding:"1.5rem",maxHeight:"90vh",overflowY:"auto" }}>
        ...
        <div style={{ background:"#f8faff",border:"1px solid #e0e7ff",borderRadius:".75rem",padding:"1.1rem 1.15rem",marginBottom:"1rem" }}>
          ...
          <div style={{ display:"flex",gap:".6rem" }}>
            <input type="password" value={claudeKeyDraft} onChange={e=>setClaudeKeyDraft(e.target.value)} placeholder="sk-ant-…" className="field-input" style={{ flex:1,fontFamily:"monospace",fontSize:".82rem" }}/>
            <button onClick={saveClaudeKey} className="btn-primary" style={{ flexShrink:0,padding:".6rem 1rem" }}><SaveIcon size={14}/> Save</button>
          </div>
        </div>
        ...
        <div style={{ marginTop:"1.15rem",display:"flex",justifyContent:"flex-end",gap:".75rem" }}>
          <button onClick={()=>{ onSave({ claudeApiKey, webhookUrl, customPRPrompt: customPromptDraft }); showToast("Settings saved!"); onClose(); }} className="btn-primary"><SaveIcon size={14}/> Save All</button>
